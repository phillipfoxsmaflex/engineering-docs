...
# Approach 1: Try connecting as postgres user with environment variables
if PGPASSWORD="${DB_PASSWORD:-apppassword}" psql -h db -U "${DB_USER:-postgres}" -d "${DB_NAME:-engineering_docs}" -c "SELECT 1;" >/dev/null 2>&1; then
    echo "Connected as ${DB_USER:-postgres} user"
    DB_CONNECTED=true
    SUPERUSER="${DB_USER:-postgres}"
    SUPERUSER_PASSWORD="${DB_PASSWORD:-apppassword}"
elif PGPASSWORD="${APP_DB_PASSWORD:-apppassword}" psql -h db -U "${APP_DB_USER:-appuser}" -d "${DB_NAME:-engineering_docs}" -c "SELECT 1;" >/dev/null 2>&1; then
    echo "Connected as ${APP_DB_USER:-appuser} (already exists)"
    DB_CONNECTED=true
    echo "${APP_DB_USER:-appuser} already exists and is functional"
    exit 0
else
    ...
    for user in postgres admin root; do
        for password in postgres_password postgres admin root ""; do
            if [ -n "$password" ]; then
                if PGPASSWORD="$password" psql -h db -U "$user" -d "${DB_NAME:-engineering_docs}" -c "SELECT 1;" >/dev/null 2>&1; then
                ...
                if psql -h db -U "$user" -d "${DB_NAME:-engineering_docs}" -c "SELECT 1;" >/dev/null 2>&1; then
...
echo "Creating ${APP_DB_USER:-appuser} role using $SUPERUSER..."
if [ -n "$SUPERUSER_PASSWORD" ]; then
    PGPASSWORD="$SUPERUSER_PASSWORD" psql -h db -U "$SUPERUSER" -d "${DB_NAME:-engineering_docs}" -c "
    DO \$\$
    BEGIN
